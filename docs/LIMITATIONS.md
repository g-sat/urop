...
**Discovery.** Empty `urls` triggers Wikipedia / allowlist search. Biases toward English reference sites. Scaled by `DISCOVERY_SCORE_FACTOR` (default `0.55`). Fallback path, not a substitute for real cites.
...
**Claim → URL routing.** Linked-answer script uses overlap + topic hints. Fixed UNESCO→engagingplaces once; new domains need new hints or a better aligner.
