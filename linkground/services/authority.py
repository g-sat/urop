"""Open PageRank authority lookup with caching."""
...
async def fetch_authority_multipliers(urls: List[str]) -> List[float]:
    """
    Return one authority multiplier per URL.

    Uses memory cache, then disk cache, then a single bulk Open PageRank request
    for any missing domains.
    """
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"[authority] Open PageRank HTTP {response.status_code}: "
                        f"{response.text[:200]!r}"
                    )
            except Exception as exc:  # noqa: BLE001 - network failures must not crash API
