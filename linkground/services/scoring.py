"""Continuous groundedness parsing and statement normalization."""
...
def parse_groundedness(model_output: str, expected_claims: int = 1) -> float:
    """
    Extract a continuous groundedness score in [0, 1].

    Preference:
      1. GROUNDEDNESS: 0.xx
      2. Mean of SUPPORT: 0.xx claim lines
      3. Legacy TRUE/FALSE mapping
    """
    ...
    for raw_line in text.splitlines():
        ...
        support_match = re.search(r"SUPPORT\s*[:=]\s*([01](?:\.\d+)?)", line, flags=re.IGNORECASE)
        if support_match:
            support_scores.append(clamp01(float(support_match.group(1))))
            continue

        arrow_score = re.search(r"->\s*([01](?:\.\d+)?)\b", line)
