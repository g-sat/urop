...
def load_metrics() -> pd.DataFrame:
    if not METRICS_CSV.exists():
        raise FileNotFoundError(f'No metrics CSV found at {METRICS_CSV}. Run scripts/run_benchmark.py first.')
...
def generate_report() -> None:
    ...
    with open(REPORT_PATH, 'w', encoding='utf-8') as report:
        ...
        for model_name in frame['model'].unique():
            ...
            f1 = f1_score(subset['expected_class'], subset['predicted_class'], average='weighted', labels=labels, zero_division=0)
            precision = precision_score(subset['expected_class'], subset['predicted_class'], average='weighted', labels=labels, zero_division=0)
            recall = recall_score(subset['expected_class'], subset['predicted_class'], average='weighted', labels=labels, zero_division=0)
            tradeoff_rows.append({'model': str(model_name).upper(), 'latency': latency, 'mae': mae, 'f1_weighted': f1})
            ...
            if 'trust_index' in subset.columns:
                ...
                report.write(f'Trust-index MAE                        : {float(np.abs(trust_errors).mean()):.4f}\n')
                report.write(f'Trust-index RMSE                       : {float(np.sqrt((trust_errors ** 2).mean())):.4f}\n')
            if 'authority_multiplier' in subset.columns:
                report.write(f"Mean link authority                    : {subset['authority_multiplier'].mean():.4f}\n")
            ...
            report.write(classification_report(subset['expected_class'], subset['predicted_class'], labels=labels, zero_division=0))
            report.write('\n' + '=' * 72 + '\n\n')
            matrix = confusion_matrix(subset['expected_class'], subset['predicted_class'], labels=labels)
        ...
        sns.scatterplot(data=tradeoff, x='latency', y='mae', hue='model', style='model', s=280, palette='Set1')
        for _, row in tradeoff.iterrows():
            plt.text(row['latency'] + 0.05, row['mae'], f"{row['model']} (MAE={row['mae']:.3f})", fontsize=9)
        ...
        plt.figure(figsize=(7, 5))
        sns.scatterplot(data=tradeoff, x='latency', y='f1_weighted', hue='model', style='model', s=280, palette='Set1')
        for _, row in tradeoff.iterrows():
            plt.text(row['latency'] + 0.05, row['f1_weighted'] - 0.002, f"{row['model']} (F1={row['f1_weighted']:.2%})", fontsize=9)
